...
<!-- Code for Day 2. Assumes students have done the RStudio and Code tutorial. We allow them to make any plot they want. Does not have to be this one. (Indeed, overlapping densities would probably be better.) Anything "interesting" which looks at US heights. Also want to end the class by putting the image up on Rpubs and cleaning it up, by, for example, removing everything from the YALK except the "output: html_document". They should have learned about code chunk options (but probably not enough). Important to highlight the need for code comments and figure labs. -->
